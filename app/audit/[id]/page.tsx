...
export default async function SourceAuditReportPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // v0.7: load audit history for the same brand, then compute drift vs the
  // most recent prior audit (if any).
  ...
  return (
    <main>
      ...
      {report.competitors && report.competitors.competitors.length > 0 && (
        <CompetitorSection competitors={report.competitors} brand={report.brand} />
      )}

      {/* DRIFT — Day-N vs prior audit for the same brand (v0.7) */}
...
function CompetitorSection({
  ...
  const allSightings = competitors.sightings;
  const maxCount = Math.max(1, ...allSightings.map((s) => s.urlCount));
  return (
    <section className="border-b border-rule">
      <div className="max-w-8xl mx-auto px-8 py-20">
        <div className="grid grid-cols-12 gap-x-6 mb-10">
          ...
          <div className="col-span-12 md:col-span-10">
            ...
            <p className="mt-3 text-base text-muted max-w-3xl">
              When a buyer asks one of the 10 prompts above, the engine returns URLs and a
              short answer. We matched each citation against known competitors for this
              category. <span className="text-ink font-data">Share of voice</span> is the brand's
              share of all citations (brand + competitors).
              {visibleSightings.length === 0 && (
                <span className="block mt-2 text-ok">
                  No competitor appeared in the cited set this run — either {brand} dominates
                  the category here, or the engine returned no sources at all.
                </span>
              )}
            </p>
          </div>
        ...
        <div className="grid grid-cols-12 gap-x-6 mb-10">
          <div className="col-span-12 md:col-span-3 border-r border-rule pr-6">
            <p className="eyebrow text-muted mb-2">Brand share of voice</p>
            <p className="font-display text-6xl text-signal leading-none"
               style={{ fontWeight: 580, fontVariationSettings: "'opsz' 144, 'SOFT' 30" }}>
              {sovPct}<span className="text-2xl text-muted">%</span>
            </p>
            <p className="text-xs text-muted font-data mt-2">
              {competitors.totalBrandCitations} brand cites / {competitors.totalBrandCitations + competitors.totalCompetitorCitations} total
            </p>
          </div>
          <div className="col-span-12 md:col-span-3 border-r border-rule md:px-6">
            ...
            <p className="font-display text-6xl text-ink leading-none"
               style={{ fontWeight: 580, fontVariationSettings: "'opsz' 144, 'SOFT' 30" }}>
              {competitors.competitors.length}<span className="text-2xl text-muted"> brands</span>
            </p>
          ...
          <div className="col-span-12 md:col-span-3 md:px-6">
            ...
            <p className="text-xs text-muted font-data mt-2">
              competitors appeared in cited URLs or text
            </p>
          </div>
        </div>
            ...
            return (
              <li key={s.name}
                ...
                <span className="col-span-12 md:col-span-3 md:text-right space-y-1">
                  ...
                  {s.textMention && (
                    <span className="block text-xs uppercase tracking-eyebrow text-signal">
                      ✓ named in text
                    </span>
                  )}
                </span>
        ...
        <p className="mt-8 text-xs text-muted font-data max-w-3xl leading-relaxed">
          v0.7 competitor comparison. Seeded from a category dictionary — for paid engagements
          we replace this with a custom watchlist of competitors the buyer names, plus we run
          a Wikipedia-cooccurrence pass to surface up-and-coming players the seed list misses.
        </p>
      </div>
    </section>
  );
      ...
      <div className="max-w-8xl mx-auto px-8 py-20">
        <div className="grid grid-cols-12 gap-x-6 mb-10">
          ...
          <div className="col-span-12 md:col-span-10">
            ...
            <p className="mt-3 text-base text-muted max-w-3xl">
              Comparing this report against the prior audit for the same brand. Day-90
              engagements re-run on this cadence to show what the work moved.
            </p>
          </div>
        </div>
